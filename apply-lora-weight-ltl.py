...
def get_args():
  ...
  parser.add_argument("--peft_model_weights", type=str)
  parser.add_argument("--output_dir", type=str)
  parser.add_argument("--cpu_mode", action="store_true")
  return parser.parse_args()

def merge(base_model, lora_model, scaling, merge_weights):
    weights_list = []
    ...
    print(f"Found {len(weights_list)} weights to merge")
    for (name,weight,a,b) in weights_list:
        layer_number  = int(name.split('base_model.model.model.layers.')[1].split('.')[0])
        merge_weight = merge_weights[layer_number]
        ab = b @ a
        weight += ab * scaling * merge_weight
        print(f"Did thing for layer named {name}, merge weight {merge_weight} and layer number {layer_number}")
...
def initiate_model_lora_merge(model_path, lora_path, output_dir, merge_weights, cpu_mode=False):
    print(model_path)
    print(lora_path)
    ...
    merge(base_model, lora_model, scaling, merge_weights=merge_weights)
...
def main():
  ...
  args = get_args()
  merges_weights = [float(i) for i in args.peft_model_weights.split(',')]
  initiate_model_lora_merge(args.base_model_name_or_path, args.peft_model_path, args.output_dir, merges_weights, args.cpu_mode)
